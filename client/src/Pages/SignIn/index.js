    ...
    postData("/api/user/signin", { ...formfields, tempUserId }).then((res) => {
      try {
        if (res.error !== true) {
          ...
          // Determine the redirect path based on tempUserId
          const redirectPath = tempUserId ? "/cart" : "/";
          
          // Remove tempUserId from localStorage
          ...
          setTimeout(() => {
            history(redirectPath);  // Use the determined redirect path
