...
router.get(`/`, async (req, res) => {
    try {
        ...
        // Base query to exclude Bajaj products
        const baseQuery = { brand: { $ne: "Bajaj" } };

        // Count total products (excluding Bajaj)
        const totalPosts = await Product.countDocuments(baseQuery);
...
router.get(`/featured`, async (req, res) => {
    try {
        const productList = await Product.find({ catName: "Bajaj" });
      
        return res.status(200).json(productList);
    } catch (error) {
        console.error("Error fetching featured Bajaj products:", error);
        return res.status(500).json({ success: false, message: "Internal server error" });
    }
});
